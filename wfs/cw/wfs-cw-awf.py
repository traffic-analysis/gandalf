...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("DataPreprocess"):
            ...
            all_y = subf_data['y']
            # shuffling
            train_x, train_y = shuffle(all_x, all_y)
        ...
        with tf.name_scope("InputPipeline"):
            ...
            for i in range(args.num_classes):
                print(i)
                train_x_labeled.append(X[y == i][:int(args.num_labeled_examples)])
                train_y_labeled.append(y[y == i][:int(args.num_labeled_examples)])
                test_x_data.append(X[y == i][int(args.num_labeled_examples):int(args.num_labeled_examples) + 96*5])
                test_y_data.append(y[y == i][int(args.num_labeled_examples):int(args.num_labeled_examples) + 96*5])
        ...
        writer = tf.summary.FileWriter(_next_logdir("tensorboard/wfs50_cw_awf"))
        ...
        print('steps_per_test', steps_per_test)
...
if __name__ == "__main__":
    ...
    parser.add_argument ('--train_epochs', required=False, default=4)
    ...
    parser.add_argument ('--num_labeled_examples', required=False, default=50)
