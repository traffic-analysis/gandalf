...
"""
DF set (circuit lable): [circuit_index*25 + i for i in range(25)] for circuit_index:0-39
...
def main(args):
    ...
    with tf.Graph ().as_default ():
        ...
        with tf.name_scope ("DataPreprocess"):
            ...
            if int(args.num_train_circuits) < 36:
                circuit_idx = np.array (range (40))  # total 40 circuits
                np.random.shuffle (circuit_idx)
                circuit_train_idx = circuit_idx[:int(args.num_train_circuits)]
                circuit_test_idx = [idx for idx in range (1000) if idx not in circuit_train_idx]
                train_x = []
                train_y = []
                for circuit_index in circuit_train_idx:
                    x = list (all_x[all_c == circuit_index])
                    y = list (all_y[all_c == circuit_index])
                    train_x += x
                    train_y += y
                ...
                test_x_data = []
                test_y_data = []
                for circuit_index in circuit_test_idx:
                    x = list (all_x[all_c == circuit_index])
                    y = list (all_y[all_c == circuit_index])
                    test_x_data += x
                    test_y_data += y
                ...
                test_x_data = test_x_data[:int (len (all_y) * 0.1)]
                test_y_data = test_y_data[:int (len (all_y) * 0.1)]  # 9500
            else:
                all_x, all_y = shuffle (all_x, all_y)
                train_x = all_x[:int (len (all_x) * 0.9)]
                train_y = all_y[:int (len (all_x) * 0.9)]
                test_x_data = all_x[int (len (all_x) * 0.9):]
                test_y_data = all_y[int (len (all_x) * 0.9):]
        ...
        writer = tf.summary.FileWriter (_next_logdir ("tensorboard/wfi_circuit"))
